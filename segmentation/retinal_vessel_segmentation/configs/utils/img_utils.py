...
def get_test_patches1(img,prob,config,rl=False):
    """
    将待分割图预处理后，分割成patch
    :param img: 待分割图
    :param config: 配置文件
    ...
    test_img = []
    prob_img = []
    prob_img.append(prob)
    test_img.append(img)
    
    test_img=np.asarray(test_img)
    prob_img=np.asarray(prob_img)
    test_img_adjust=img_process1(test_img,prob_img,rl=rl)  #预处理
    test_imgs=paint_border(test_img_adjust,config)  #将图片补足到可被完美分割状态
...
#            train_img = medianBlur(train_img)
...
def img_process1(data,data2,rl=False):
    """
    预处理图片
    :param data: 输入图片
    :param rl: 原始图片是否预处理过
...
#            train_img = medianBlur(train_img)
            ...
            train_imgs = np.concatenate([train_imgs,data2],axis=1)
...
def medianBlur(imgs):
    ...
    assert (imgs.shape[1]==1)  #check the channel is 1
    #create a CLAHE object (Arguments are optional).
    imgs_median = np.empty(imgs.shape)
    for i in range(imgs.shape[0]):
        imgs_median[i,0] = cv2.medianBlur(np.array(imgs[i,0], dtype = np.uint8),5)
    return imgs_median   
