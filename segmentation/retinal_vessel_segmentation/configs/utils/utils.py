...
#    print(masks[0,:,0,0])
    ...
    new_masks = np.zeros((masks.shape[0], im_h * im_w, channels + 1))
    for i in range(masks.shape[1]):
#        print(masks.shape[1])
#        print(i)
        index = int(masks[0,i,0])
        
        new_masks[0,i,index]=1
...
def genMasks(masks,channels):
    ...
    for index in range(channels):
        ...
        m = np.ma.array(new_masks[:, :, index], mask=mask)
        maskTotal=maskTotal|m.mask
#        print(maskTotal.shape)
    new_masks[:, :, channels] = 1 - maskTotal
    return new_masks

def genMasks1(masks,channels):
    """
    为groundtruth生成相反的掩膜
    generate masks for groundtruth
    :param masks:  groundtruth图
